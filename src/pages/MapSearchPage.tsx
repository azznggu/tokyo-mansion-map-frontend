...
import { useMansionsInBounds } from '../hooks';
import { useSearchStore } from '../store';
import type { Mansion } from '../types';

export const MapSearchPage = () => {
  // 선택된 좌표의 물건들 (복수 가능)
  const [selectedMansions, setSelectedMansions] = useState<Mansion[]>([]);
  const { setSelectedMansion: setStoreSelectedMansion, mapBounds, filter } = useSearchStore();
  const { data: mansions = [], isLoading } = useMansionsInBounds(mapBounds, filter);

  // 좌표별로 물건 그룹화
  const groupedMansions = useMemo(() => {
    const groups = new Map<string, Mansion[]>();
    mansions.forEach((mansion) => {
      const key = `${mansion.latitude},${mansion.longitude}`;
      if (!groups.has(key)) {
        groups.set(key, []);
      }
      groups.get(key)!.push(mansion);
    });
    return groups;
  }, [mansions]);

  // 마커 클릭 시 해당 좌표의 모든 물건 선택
  const handleSelectMansion = useCallback(
    (mansion: Mansion) => {
      const key = `${mansion.latitude},${mansion.longitude}`;
      const mansionsAtLocation = groupedMansions.get(key) || [mansion];
      setSelectedMansions(mansionsAtLocation);
      setStoreSelectedMansion(mansion.id);
    },
    [setStoreSelectedMansion, groupedMansions]
  );
  ...
  return (
    <div className="flex h-full w-full">
        ...
        <div className="hidden md:block w-96 flex-shrink-0 bg-white border-r border-gray-200 overflow-y-auto">
          <div className="p-4">
            ...
            <div className="space-y-4">
              {selectedMansions.map((mansion, index) => (
                <div key={mansion.id} className="border-b border-gray-100 pb-4 last:border-b-0">
                  {selectedMansions.length > 1 && (
                    <div className="text-xs text-gray-500 mb-2">物件 {index + 1}</div>
                  )}
                  <MansionCard mansion={mansion} isSelected />
                  ...
                  <div className="mt-3">
                    <div className="bg-gray-50 rounded-lg p-3 mb-3">
                      ...
                      <ul className="text-xs text-gray-600">
                        {mansion.stations.map((station, idx) => (
                    ...
                    <div className="bg-gray-50 rounded-lg p-3 mb-3">
                      <h4 className="font-medium text-gray-900 mb-2 text-sm">建物概要</h4>
                      <dl className="text-xs">
                        <div className="flex justify-between mb-1">
                          <dt className="text-gray-500">総戸数</dt>
                          <dd className="text-gray-900">{mansion.totalUnits}戸</dd>
                        </div>
                        <div className="flex justify-between mb-1">
                          <dt className="text-gray-500">階数</dt>
                          <dd className="text-gray-900">{mansion.floors}階建て</dd>
                        </div>
                        <div className="flex justify-between mb-1">
                          <dt className="text-gray-500">完成予定</dt>
                          <dd className="text-gray-900">{mansion.completion}</dd>
                        </div>
                        <div className="flex justify-between">
                          <dt className="text-gray-500">売主</dt>
                          <dd className="text-gray-900">{mansion.developer || '-'}</dd>
                        </div>
                      </dl>
                    </div>

                    <a
                      href={mansion.sourceUrl}
                      target="_blank"
      ...
      <div className="flex-1 relative h-full">
        ...
        <MansionMap
          mansions={mansions}
          groupedMansions={groupedMansions}
          selectedMansion={selectedMansions[0] || null}
          ...
          <div className="flex flex-row items-center gap-3">
            <div className="bg-white shadow-lg rounded-lg px-4 py-3 border border-gray-200">
              <span className="text-base text-gray-600">
                検索結果: <span className="font-bold text-blue-600 text-lg">{mansions.length}</span>件
              </span>
            </div>
            <SearchFilter />
          </div>
        ...
        <div className="md:hidden fixed bottom-6 right-6 z-[500] flex flex-col items-end gap-3">
          <div className="bg-white shadow-lg rounded-lg px-4 py-3 border border-gray-200">
            <span className="text-sm text-gray-600">
              検索結果: <span className="font-bold text-blue-600 text-base">{mansions.length}</span>件
            </span>
          </div>
